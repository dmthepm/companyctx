...
- Artifact id: `brief-6a24fd6cb2dc` (opaque sha256 prefix — the partner
...
`cary-hormone-02.example` / "Cary Hormone 02" and any personal name to "the Owner";
...
- Extractor change that decodes sanitized markup differently →
  `test_envelope_matches_hand_curated_oracle[cary-hormone-02]` fails.
- Provider-JSON fixture edit that diverges from the captured
  observations → `test_{google,yelp,youtube}_fixture_matches_observations[cary-hormone-02]`
  fails.
