...
pipeline processed on `2026-04-12`. The oracle (`expected.json`) is frozen;
...
- Artifact date: `2026-04-12`
- Artifact id: `brief-b2f68716f981` (opaque sha256 prefix — the partner
...
`birmingham-iv-04.example` / "Birmingham Iv 04" and any personal name to "the Owner";
...
- Extractor change that decodes sanitized markup differently →
  `test_envelope_matches_hand_curated_oracle[birmingham-iv-04]` fails.
- Provider-JSON fixture edit that diverges from the captured
  observations → `test_{google,yelp,youtube}_fixture_matches_observations[birmingham-iv-04]`
  fails.
