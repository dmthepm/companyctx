...
- Artifact id: `brief-28a610bddf4e` (opaque sha256 prefix — the partner
...
`charleston-medspa-05.example` / "Charleston Medspa 05" and any personal name to "the Owner";
...
- Extractor change that decodes sanitized markup differently →
  `test_envelope_matches_hand_curated_oracle[charleston-medspa-05]` fails.
- Provider-JSON fixture edit that diverges from the captured
  observations → `test_{google,yelp,youtube}_fixture_matches_observations[charleston-medspa-05]`
  fails.
